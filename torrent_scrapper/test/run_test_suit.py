...
class Wrappers(unittest.TestCase):
    ...
    def test_vertor_wrapper(self):
        ...
        te = TorrentEntry(
            ...
            file_name="TestTorrentVertor.torrent"
        )
        wrapper.download_torrent(te)
        pathToFile = os.path.join(os.path.abspath("."), te.file_name)
        self.assertTrue(os.path.exists(pathToFile))
        self.filesToDelete.append(pathToFile)

    def no_test_kickass_wrapper(self):
        """
        ...
        te = TorrentEntry(
            ...
            file_name="TestTorrentKickAss.torrent"
        )
        wrapper.download_torrent(te)
        pathToFile = os.path.join(os.path.abspath("."), te.file_name)
        self.assertTrue(os.path.exists(pathToFile))
    ...
    def test_thound_wrapper(self):
        ...
        te = TorrentEntry(
            ...
            file_name="TestTorrentThound.torrent"
        )
        wrapper.download_torrent(te)
        pathToFile = os.path.join(os.path.abspath("."), te.file_name)
        self.assertTrue(os.path.exists(pathToFile))
